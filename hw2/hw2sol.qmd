...
$$ 
\begin{aligned}
    P(x)(1+^{\beta_0 + \beta_1x}) &= e^{\beta_0 + \beta_1x} \Rightarrow \\
    (1+^{\beta_0 + \beta_1x} &= \frac {1} {\frac{1}{1 + e^{\beta_0 + \beta_1x}}}) \\
    \frac{P(x)}{\frac{1}{1 + e^{\beta_0 + \beta_1x}}} &= e^{\beta_0 + \beta_1x} \Rightarrow\\
    \frac{P(x)}{1 - \frac{e^{\beta_0 + \beta_1x}}{1+ e^{\beta_0 + \beta_1x}}} &= e^{\beta_0 + \beta_1x} \Rightarrow\\
    \frac{P(x)}{1 - P(x)} &= e^{\beta_0 + \beta_1x} \\
\end{aligned} 
$$ 
Therefore, the logistic function representation and logit representation for the 
...
$$
\begin{aligned}
\log(\frac{p(X)}{1 - p(X)}) = \hat\beta_0 + \hat\beta_0*X_1 + \hat\beta_2*X_2\\
Assign \ the \ values \ of \ p(X), \ X_1, \ X_2, \ \hat\beta_0, \ \hat\beta_0, \ \hat\beta_2 \ to \ the \ equation,\\
\log(\frac{0.5}{1 - 0.5}) = -6 + 0.05*X_1 + 1*3.5\\
Solve \ the \ equation: \ X_1 = 50 \ hours. 
\end{aligned}
$$
...
The median and mean values for these lagged variables are close to zero and 
almost identical, indicating that there is no significant overall upward or 
downward trend in the percentage return.
...
summary(model)
```
Lag2 appears to be statistically significant. The p-value for Lag2 is less than 
0.05.
...
"Up" (column 2). There are 430 occurrences of this. In the context of stock 
market prediction, this means the model falsely predicted an "Up" movement when 
the actual movement was "Down."

**False Negatives (Type II Error):**

Instances where the actual direction was "Up" (row 2), but the model predicted 
"Down" (column 1). There are 48 occurrences of this. In the context of stock 
market prediction, this means the model falsely predicted a "Down" movement when 
the actual movement was "Up."
...
"Down" movements when they are predicted to be "up" (false negatives) than with 
predicting "Up" movements when they are predicted to be "Down" (false positives).
...
```
Above is the confusion matrix and the overall fraction of correct predictions
for the held out data is 0.5865385.
...
```
Above is the confusion matrix and the overall fraction of correct predictions
for the held out data is 0.5096154.
...
lag1 and lag2, lag3 and lag2, lag4 and lag2, lag5 and lag2, volume and lag2 to 
see if there are interactions between them. 

And keeping lag2 always as one of the predictors, adding 2 more variables to see 
if there are interactions between them. 

Then, I will add 3 more variables. 

Finally, I will add 4 more variables. 
...
```{r}
#| eval = TRUE
# Fit LDAn with Lag2 and another lag as the predictors
...
```{r}
#| eval = TRUE
# Fit LDA with Lag2 and another lag as the predictors
...
If $x∈[0.05,0.95]$, then the observations we will use are in the interval 
$[x−0.05,x+0.05]$, which represents a length of 0.1, which represents a fraction 
of 10%.

If x<0.05, then we will use observations in the interval $[0, x+0.05]$, which 
represents a fraction of $(100x+5)%$.

If x>0.95, we will use observations in the interval $[x-0.05, 1]$, which 
represents a fraction of $(105−100x)%$.

To compute the average fraction we will use to make the prediction we have to
calculate the integrate of the fraction of the available observations over the
range of X as follows:
$$
\begin{aligned}
\int_{0}^{0.05} (100x+5)dx + \int_{0.05}^{0.95} 10dx + 
\int_{0.95}^{1} (105−100x)dx 
&= \left(50x^2+5x\right)|_{0}^{0.05} + \left(10x\right)|_{0.05}^{0.95} + 
\left(105x−50x^2\right)|_{0.95}^{1} \\
&= 50*(0.05)^2+5*(0.05) + 10*(0.95−0.05) + 105*1−50*1^2−
\left(105*0.95-50*0.95^2\right) \\
&= 0.25+0.5+9.5+105−50−105+47.5 \\
&= 9.75
\end{aligned}
$$
Therefore, On average, we will use 9.75% of the available observations to make
the prediction.
...
If we assume X1 and X2 to be independent, the fraction of available observations 
we will use to make the prediction is the product of the fraction of available
observations for X1 and X2, which is
$$
9.75\%×9.75\% = 0.950625\%
$$
Therefore, the fraction of available observations we will use to make the
prediction is 0.950625%.
...
Based on (a) and (b), we can conclude that the fraction of available 
observations we will use to make the prediction is the product of the fraction
of available observations for each feature, which is
$$
9.75 \% ^{100} \approx 0 \%
$$
Therefore, the fraction of available observations we will use to make the 
prediction is approximately 0%.
...
Based on parts (a)-(c), we can conclued that the fraction of available 
observations we will use to make the prediction is $(9.75%)^p$, where $p$ is the 
number of features. So when p is infinitely large, the fraction of available
observations we will use to make the prediction is 0. Therefore, there are very
few training observations "near" any given test observation.
$$
0<9.75\%<1 \Rightarrow
\lim_{p\rightarrow∞} (9.75\%)^p=0.
$$
...
For p=1, the center of a line segment is its midpoint. Think of observations 
queuing evenly on ths line segment and therefore we have length of the line 
segment equal to $0.1$.

For p=2, the center of the square is the point where the two diagonals of the 
square intersect, and these two diagonals are perpendicular to each other. 
Consequently, we have a length equal to $0.1^{1/2}$.

For p=100, the center of the 100-dimensional cube is the point where the
diagonals of the cube intersect, and these diagonals are perpendicular to each
other. Consequently, we have a length equal to $0.1^{1/100}$.
