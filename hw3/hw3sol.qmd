...
available. When n is relatively small compared to p, the best subset might 
...
stepwise selection.
### Answer: FALSE
In backward stepwise selection, the algorithm starts with the full set of 
predictors and iteratively removes the least significant predictor until the 
desired number of predictors is reached. In forward stepwise selection, the 
algorithm starts with an empty set and adds predictors one by one based on their 
significance.
...
stepwise selection.
### Answer: FALSE
Likewise, in forward stepwise selection, the algorithm starts with an empty set
and adds predictors one by one based on their significance. In backward stepwise
selection, the algorithm starts with the full set of predictors and iteratively
removes the least significant predictor until the desired number of predictors 
is reached.
...
selection.
### Answer: FALSE
In best subset selection, the k-variable model identified does not necessarily 
have predictors that are a strict subset of the (k + 1)-variable model. In best 
subset selection, models of different sizes (with different numbers of 
predictors) are considered, and the best model for each size is chosen based on 
some criteria (e.g., minimizing the sum of squared errors). This means that the 
predictors in the k-variable model might not be a strict subset of the 
predictors in the (k + 1)-variable model. The algorithm evaluates all possible 
combinations of predictors and selects the best model for each size 
independently, so the selected predictors can vary between models of different 
sizes.
