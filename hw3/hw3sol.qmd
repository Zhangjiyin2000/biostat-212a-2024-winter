...
```{r}
# Assuming .metric contains metric names and .estimate contains corresponding values
ggplot(cv_metrics, aes(x = n, y = .estimator, group = .metric, color = .metric)) +
  geom_line() +
  labs(title = "Cross-Validation Results for Least Squares Model", x = "Number of Folds", y = "RMSE") +
  theme_minimal()
```
...
results_df <- tibble(
  ...
  CV_RMSE = c(mean(cv_rmse_ls), mean(cv_rmse_ridge), mean(cv_rmse_lasso)),
