...
Least squares:
```{r}
ls_final_wf <- ls_lr_wf %>%
  finalize_workflow(best_ls)
ls_final_wf
...
test_rmse_ls <- ls_final_fit %>% collect_metrics() %>% filter(.metric == "rmse") %>% select(.estimate)
test_rmse_ls
...
test_rmse_ridge <- ridge_final_fit %>% collect_metrics() %>% filter(.metric == "rmse") %>% pull(.estimate)
test_rmse_ridge
```
Lasso:
```{r}
lasso_final_fit %>% collect_metrics()
test_rmse_lasso <- lasso_final_fit %>% collect_metrics() %>% filter(.metric == "rmse") %>% pull(.estimate)
test_rmse_lasso
```

```{r}
# Assuming you have vectors or data frames with CV RMSE and Test RMSE for each method
#cv_rmse_ls <- ...
#test_rmse_ls <- ...
...
results_df <- tibble(
  ...
  CV_RMSE = c(mean(cv_rmse_ridge), mean(cv_rmse_lasso)),
  Test_RMSE = c( test_rmse_ridge, test_rmse_lasso)
)
...
Assuming that $N > M$, as the training data set is larger than the test data set.
...
&= \operatorname{E}\left[(y_1 - \hat{\beta}^T x_1)^2\right]
\end{align*}
$$
